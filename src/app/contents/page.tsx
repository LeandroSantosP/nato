...
export default function Contents() {
	...
	const { data, isLoading } = useQuery({
		...
		queryFn: () => category_name != null ? get_categories_by_name({ category_name, video_title: video_title ?? "" }) : get_categories()
	});
	return (
		<main className="space-y-0.5 gap-2 p-2 w-full flexx flex-col h-screen">
			<FilterCategoriesForm />
			{isLoading ? <SkeletonLoadCategories /> : (
				<section className="flex overflow-auto h-full flex-col gap-10 p-3 scrollbar-thin scrollbar-thumb-emerald-500 scrollbar-track-zinc-800">
					{data?.map(category => {
						return (
							<div key={category.id} className="flex flex-col ">
								...
								<div className="flex w-full flex-wrap h-full gap-2 my-6">
